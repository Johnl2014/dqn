...
class LinearDecayGreedyEpsilonPolicy(Policy):
    ...
    def select_action(self, q_values, is_training=True, epsilon=0.05):
    # def select_action(self, q_values, is_training=True, epsilon=0.05, **kwargs):
        ...
        """
        if is_training:
            # epsilon = max(self.end_value, self.start_value - (self.start_value
                      # - self.end_value) * curr_step / num_steps)
            epsilon = max(self.epsilon, self.end_value)
            self.epsilon += self.epsilon_step
        # else:
        #     print('not training and use default epsilon', epsilon)
